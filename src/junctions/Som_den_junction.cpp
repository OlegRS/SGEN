...
Junction& Neuron::Som_den_junction::set_hopping_rate_constants() {
  ...
  double
    ...
    mRNA_bkwd_diff_rate = p_to->mRNA_diffusion_constant * 3600 * diffusion_scaling_factor,
    mRNA_fwd_traff_rate = p_from->mRNA_forward_trafficking_velocity * 3600 * trafficking_scaling_factor,
    mRNA_bkwd_traff_rate = p_to->mRNA_backward_trafficking_velocity * 3600 * trafficking_scaling_factor,
    ...
    prot_fwd_traff_rate = p_from->protein_forward_trafficking_velocity * 3600 * trafficking_scaling_factor,
    prot_bkwd_traff_rate = p_to->protein_backward_trafficking_velocity * 3600 * trafficking_scaling_factor;
    
  fwd_mRNA_hop_rate = (mRNA_fwd_diff_rate + mRNA_fwd_traff_rate);///static_cast<Soma*>(p_from)->n_descending_DS;
  bkwd_mRNA_hop_rate = mRNA_bkwd_diff_rate + mRNA_bkwd_traff_rate;

  fwd_prot_hop_rate = (prot_fwd_diff_rate + prot_fwd_traff_rate);///static_cast<Soma*>(p_from)->n_descending_DS;
